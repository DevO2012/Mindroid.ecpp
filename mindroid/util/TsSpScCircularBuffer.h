...
public:
	...
	uint16_t front();
	uint16_t front(void* data, uint16_t size);
...
template<uint16_t SIZE>
uint16_t TsSpScCircularBuffer<SIZE>::front(void* data, uint16_t size) {
	if (data == NULL) {
		return 0;
	}
	...
	uint16_t dataSize;
	readData(readIndex, (uint8_t*) &dataSize, 2);
	uint16_t curSize = (size > dataSize) ? dataSize : size;
	readData((readIndex + 2) % SIZE, (uint8_t*) data, curSize);
	return curSize;
}
